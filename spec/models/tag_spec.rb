...
RSpec.describe Tag, type: :model do
  ...
  context "numbers" do
    ...
    let!(:tag10) { FactoryBot.create(:tag, taxonomy: taxonomy, number: 10) }
    let!(:tag10_1) { FactoryBot.create(:tag, parent: tag10, taxonomy: taxonomy) }
    let!(:tag10_1_1) { FactoryBot.create(:tag, parent: tag10_1, taxonomy: taxonomy) }

    it "creates full numbers" do
      expect(tag1.full_number).to eq("1")
      expect(tag11.full_number).to eq("1.1")
      expect(tag111.full_number).to eq("1.1.1")
      ...
      expect(tag221.full_number).to eq("2.2.1")
      expect(tag10.full_number).to eq("10")
      expect(tag10_1.full_number).to eq("10.1")
      expect(tag10_1_1.full_number).to eq("10.1.1")
    end
    it "orders by full numbers" do
      expect(taxonomy.tags.number_order.to_a).to eq(
        [tag1, tag11, tag111, tag12, tag2, tag21, tag22, tag221, tag10, tag10_1, tag10_1_1]
      )
    end
  end
end
